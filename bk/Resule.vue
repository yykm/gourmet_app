  ...
  <div id="app">
    ...
    <div class="l-content">
      <div class="p-wrap" v-show="!loading">
        <p class="tar"><a href="https://webservice.recruit.co.jp/"><img
              src="https://webservice.recruit.co.jp/banner/hotpepper-s.gif" alt="ホットペッパー Webサービス" width="135"
              height="17" title="ホットペッパー Webサービス"></a></p>

        <div class="p-form" :class="{ 'is-fix': scrollY >= formOffset }">
          <div class="p-form-inner">
            <input type="text" name="gourmet_keyword" placeholder="フリーワード検索（店名 地名、駅名など）" class="p-keyword"
              v-model="keyword" @input="inputting">
...
<!-- *********** -->
        ...
        <div class="p-loader" :class="{ 'is-hide': getting }">
          <div></div>
          <div></div>
          <div></div>
        </div>
              ...
              <div class="p-content">
                ...
                <table class="p-table">
                  <tr>
                    <th>住所</th>
                    <td>{{ item.address }}<br>
                      <a :href="item.map" target="_blank" rel="noopener noreferrer" class="m-txt-hover p-map-link"
                        class="m-blank">Google Mapで見る</a></td>
                  </tr>
                  <tr>
                    <th>アクセス</th>
                    <td>{{ item.access }}</td>
                  ...
                  <tr>
                    <th>クーポン</th>
                    <td><a :href="item.coupon" target="_blank" rel="noopener noreferrer"
                        class="m-blank m-txt-hover">クーポンを見る</a></td>
                  </tr>
                </table>
              </div>
            </li>
  ...
  export default {
    ...
    methods: {
      ...
      getGourmet: function () {
        ...
        let $this = this;
        axios.post('【PHPファイルのパス】', params).then(function (response) { // 成功時
          $this.getting = true;
          $this.gourmetList = [];
